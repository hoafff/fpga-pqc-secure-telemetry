...
static void init_link_gpio(void) {
    ...
    /* irq_n is active low, so use pull-up while an endpoint is absent/reset. */
...
static void init_spi0(void) {
    ...
    /* CS is a GPIO because each Primer has its own select. */
...
fpst_result_t fpst_sn32f407_platform_init(fpst_platform_t *out) {
    ...
    out->fpga_reset = NULL;   /* supervisor-owned in the final topology */
    out->fpga_zeroize = NULL; /* supervisor-owned in the final topology */
