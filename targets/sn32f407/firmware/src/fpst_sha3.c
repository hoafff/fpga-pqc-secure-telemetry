...
#define FPST_MINSIZE __attribute__((minsize))
#else
#define FPST_MINSIZE
#endif
...
/*
 * The production image already links the pinned mlkem-native FIPS-202 backend.
 * Reuse that SHAKE256 entry point instead of carrying a second Keccak-f1600
 * permutation and round-constant table in the 32-KiB SN32F407F flash image.
 *
 * The symbol is generated by mlkem-native's configured namespace prefix:
 *     MLK_CONFIG_NAMESPACE_PREFIX = fpst_mlkem512_native
 */
void fpst_mlkem512_native_shake256(uint8_t *output, size_t output_len,
                                   const uint8_t *input, size_t input_len);

FPST_MINSIZE
void fpst_shake256(const uint8_t *input, size_t input_len,
                   uint8_t *output, size_t output_len) {
    fpst_mlkem512_native_shake256(output, output_len, input, input_len);
}
