...
localparam [15:0] E_MCU=16'h0701,E_PQC=16'h0702,E_CRY=16'h0703,E_TAMPER=16'h0704,E_MAN=16'h0705,E_ILLEGAL=16'h0706;
localparam [2:0] ST_MONITOR=3'd2,ST_SAFE=3'd5;
logic clk=0,rst_n=0,hb_mcu=0,hb_pqc=0,hb_crypto=0,tamper=0,clear_pulse=0,manual_fault=0;
...
fpst_supervisor_core #(.HB_TIMEOUT_MS(8),.STARTUP_ZEROIZE_MS(2),.STARTUP_GRACE_MS(4),.QUALIFY_MS(6),.ZEROIZE_HOLD_MS(2),.RESET_PULSE_MS(2),.RECOVERY_QUALIFY_MS(6),.RESET_ON_FATAL(1'b1)) dut(
 .clk_i(clk),.rst_ni(rst_n),.tick_ms_i(1'b1),.hb_mcu_i(hb_mcu),.hb_pqc_i(hb_pqc),.hb_crypto_i(hb_crypto),
...
task automatic reset_dut; begin
 rst_n=0; hb_mcu=0; hb_pqc=0; hb_crypto=0; tamper=0; clear_pulse=0; manual_fault=0; reset_pulse_seen=0;
