module primer1_pqc_btp_endpoint #(
    ...
    assign last_error_code_o = last_error_code_o_reg;

    logic [15:0] last_error_code_o_reg;
    ...
    always_comb begin
        ...
        write_coverage_next[{arg_q[0],arg_q[1]}[7:0]] = 1'b1;
    ...
    always_comb begin
        ...
        case (state_q)
            ST_WRITE_COEFF: begin
                ...
                accelerator_host_addr = {arg_q[0],arg_q[1]}[7:0];
                accelerator_host_wdata = {arg_q[2],arg_q[3]};
            end
    ...
    always_comb begin
        ...
        case (response_fill_index_q)
            ...
            default: begin
                ...
                else if (response_kind_q == RESP_BULK)
            ...
            case (state_q)
                            ...
                            case (request_opcode_i)
                                OP_PQC_WRITE_COEFF: begin
                                    if (request_payload_len_i == 16'd4)
                                        ...
                                        request_accept_o <= 1'b1;
                                        queue_response(ERR_ARGUMENT,16'h0,
                                            RESP_GENERIC,16'd0,1'b1);
                                    end
                                end
                                ...
                                OP_PQC_LOAD_POLY: begin
                                    if (request_payload_len_i >= 16'd4 &&
                                        ...
                                        request_accept_o <= 1'b1;
                                        queue_response(ERR_PQC_LENGTH,16'h0,
                                            RESP_GENERIC,16'd0,1'b1);
                                    end
                                end
                                ...
                                OP_PQC_START_NTT: begin
                                    ...
                                    if (request_payload_len_i != 16'd4)
                                        queue_response(ERR_ARGUMENT,16'h0,
                                            RESP_GENERIC,16'd0,1'b1);
                                    else if (!(&coverage_q) || domain_q != DOMAIN_STANDARD)
                                        queue_response(ERR_PQC_DOMAIN,16'h0,
                                            RESP_GENERIC,16'd0,1'b1);
                                    else if (!accelerator_host_ready || accelerator_busy_internal)
                                        queue_response(ERR_BUSY,16'h0,
                                            RESP_GENERIC,16'd0,1'b1);
                                    else begin
                                        done_latched_q <= 1'b0;
                                ...
                                OP_PQC_START_INTT: begin
                                    ...
                                    if (request_payload_len_i != 16'd4)
                                        queue_response(ERR_ARGUMENT,16'h0,
                                            RESP_GENERIC,16'd0,1'b1);
                                    else if (!(&coverage_q) || domain_q != DOMAIN_NTT)
                                        queue_response(ERR_PQC_DOMAIN,16'h0,
                                            RESP_GENERIC,16'd0,1'b1);
                                    else if (!accelerator_host_ready || accelerator_busy_internal)
                                        queue_response(ERR_BUSY,16'h0,
                                ...
                                OP_PQC_POINTWISE_MUL: begin
                                    if (request_payload_len_i != 16'd512) begin
                                        ...
                                        queue_response(ERR_PQC_LENGTH,16'h0,
                                            RESP_GENERIC,16'd0,1'b1);
                                    end else if (!(&coverage_q) || domain_q != DOMAIN_NTT) begin
                                        request_accept_o <= 1'b1;
                                        queue_response(ERR_PQC_DOMAIN,16'h0,
                                            RESP_GENERIC,16'd0,1'b1);
                                ...
                                OP_PQC_GET_RESULT: begin
                                    ...
                                    if (request_payload_len_i == 0) begin
                                        ...
                                        queue_response(ERR_OK,16'h0,
                                            RESP_INLINE,16'd8,1'b1);
                                    end else
                                        queue_response(ERR_ARGUMENT,16'h0,
                                            RESP_GENERIC,16'd0,1'b1);
                                end
                    ...
                    case (arg_kind_q)
                        ARG_WRITE_COEFF: begin
                            if ({arg_q[0],arg_q[1]} >= 16'd256)
                                ...
                                queue_response(ERR_COEFF_RANGE,16'h0,
                                    RESP_GENERIC,16'd0,1'b1);
                        ...
                        ARG_READ_COEFF: begin
                            if ({arg_q[0],arg_q[1]} >= 16'd256)
                                queue_response(ERR_ARGUMENT,16'h0,
                                    RESP_GENERIC,16'd0,1'b1);
                            else if (!coverage_q[{arg_q[0],arg_q[1]}[7:0]])
                                queue_response(ERR_INVALID_STATE,16'h0,
                                    RESP_GENERIC,16'd0,1'b1);
                            else if (!accelerator_host_ready)
                                queue_response(ERR_BUSY,16'h0,
                                    RESP_GENERIC,16'd0,1'b1);
                ...
                ST_LOAD_VALIDATE_LO: begin
                    if ({poly_hi_q,request_payload_rd_data_i} >= 16'd3329) begin
                        request_accept_o <= 1'b1;
                        queue_response(ERR_COEFF_RANGE,{7'h0,poly_index_q},
                            RESP_GENERIC,16'd0,1'b1);
                ...
                ST_LOAD_WRITE_LO: begin
                    ...
                    if (poly_index_q + 1'b1 >= poly_count_q) begin
                        request_accept_o <= 1'b1;
                        domain_q <= (poly_count_q == 9'd256)
                                  ? DOMAIN_STANDARD : DOMAIN_PARTIAL;
                        done_latched_q <= 1'b0;
                        poly_index_q <= '0;
                        queue_response(ERR_OK,16'h0,RESP_GENERIC,16'd0,1'b1);
                    end else begin
                ...
                ST_READPOLY_WAIT: begin
                    if (accelerator_host_rvalid) begin
                        ...
                        if (poly_index_q + 1'b1 >= read_poly_count_q) begin
                            poly_index_q <= '0;
                            queue_response(ERR_OK,16'h0,RESP_BULK,
                                read_poly_count_q << 1,1'b1);
                        end else begin
                            poly_index_q <= poly_index_q + 1'b1;
                            state_q <= ST_READPOLY_REQ;
                        end
                    ...
                    if (accelerator_done) begin
                        ...
                        if (accelerator_operand_error)
                            queue_response(ERR_COEFF_RANGE,
                                {8'h00,accelerator_operand_error_index},
                                RESP_GENERIC,16'd0,1'b1);
                        else begin
                            done_latched_q <= 1'b1;
