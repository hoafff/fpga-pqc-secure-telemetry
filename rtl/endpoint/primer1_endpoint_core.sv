module primer1_endpoint_core (
    ...
    /* Logical delivery acknowledgement from complete system integration.
     * No private BTP opcode is assigned here because FPST v1.1 reserves 0x61
     * for STP_RX_PACKET on the receiver endpoint. */
    ...
    /* FPST-SYS-SPEC-001 v1.1 Appendix-B registry. */
    ...
    logic [63:0] commit_initial_sequence;
    ...
    assign commit_initial_sequence = {
        cmd_buf[4], cmd_buf[5], cmd_buf[6], cmd_buf[7],
        cmd_buf[8], cmd_buf[9], cmd_buf[10], cmd_buf[11]
    };
    assign commit_policy_flags = {
        cmd_buf[12], cmd_buf[13], cmd_buf[14], cmd_buf[15]
    };
    ...
    assign endpoint_busy_o = (state_q != ST_IDLE) || ntt_busy || tx_busy;
                ...
                case (state_q)
                    ST_IDLE: begin
                        if (cmd_valid_i) begin
                            ...
                            copy_index_q <= '0;
                            state_q <= (cmd_payload_len_i == 0) ? ST_EXEC : ST_COPY;
                        end
                    end
                    ...
                    ST_EXEC: begin
                        ...
                        case (opcode_q)
                            ...
                            OP_SOFT_RESET, OP_ZEROIZE: begin
                                if (payload_len_q != 0) begin
                                    ...
                                    for (i = 0; i < 24; i = i + 1)
                                        key_material_q[i] <= 8'h00;
                                    state_q <= ST_ZEROIZE_WAIT;
                                end
                            end

                            OP_KEY_LOAD_BEGIN: begin
                                if (payload_len_q != 7 ||
                                    cmd_u32_0 == 32'h00000000 ||
                                    cmd_buf[4] != 8'h01 ||
                            ...
                            OP_KEY_LOAD_COMMIT: begin
                                if (!key_loading_q || payload_len_q != 16 ||
                                    ...
                                    response_status_q <= ERR_KEY_LOAD_INCOMPLETE;
                                    state_q <= ST_RSP_STATUS_HI;
                                end else if (commit_initial_sequence != 64'd0) begin
                                    response_status_q <= ERR_KEY_COMMIT;
                                    state_q <= ST_RSP_STATUS_HI;
                                end else begin
                                    session_stage_id_q <= cmd_u32_0;
                                    session_stage_sequence_q <= 64'd0;
                                    session_stage_policy_q <= commit_policy_flags;
                                    state_q <= ST_KEY_STAGE;
                                end
                            end
                            ...
                            OP_PQC_POLY_ADD_SUB, OP_PQC_GET_RESULT,
                    ...
                    ST_KEY_STAGE: begin
                        if (!session_stage_ready) begin
                            ...
                            state_q <= ST_KEY_WAIT_COMMIT_READY;
                        end
                    end

                    ST_KEY_WAIT_COMMIT_READY: begin
                        if (session_error_valid) begin
                            ...
                            state_q <= ST_KEY_WAIT_VALID;
                        end
                    end

                    ST_KEY_WAIT_VALID: begin
                        if (session_error_valid) begin
                            response_status_q <= session_error_code;
                            state_q <= ST_RSP_STATUS_HI;
                    ...
                    ST_RSP_STATUS_LO: begin
                        ...
                        if (response_status_q != 0 || response_len_q == 2)
                            state_q <= ST_FINISH;
                        else if (response_kind_q == RSP_NTT_READ) begin
                            ntt_index_q <= '0;
                            state_q <= ST_NTT_READ_REQ;
                        end else if (response_kind_q == RSP_TX_PACKET)
                            state_q <= ST_RSP_PACKET;
                        else
                            state_q <= ST_RSP_EXTRA;
                    end
                    ...
                    ST_RSP_PACKET: begin
                        ...
                        case (response_index_q)
                            2: rsp_payload_data_o <= tx_packet_sequence[63:56];
                            3: rsp_payload_data_o <= tx_packet_sequence[55:48];
                            4: rsp_payload_data_o <= tx_packet_sequence[47:40];
                            5: rsp_payload_data_o <= tx_packet_sequence[39:32];
                            6: rsp_payload_data_o <= tx_packet_sequence[31:24];
                            7: rsp_payload_data_o <= tx_packet_sequence[23:16];
                            8: rsp_payload_data_o <= tx_packet_sequence[15:8];
                            9: rsp_payload_data_o <= tx_packet_sequence[7:0];
                            10: rsp_payload_data_o <= tx_packet_len[15:8];
                            11: rsp_payload_data_o <= tx_packet_len[7:0];
                            default: begin
    ...
    logic unused_active_policy;
    assign unused_cmd_flags = ^cmd_flags_i;
    assign unused_active_policy = ^active_policy;
endmodule
